...
# Initialize BRIEF with default parameters
star = cv2.xfeatures2d.StarDetector_create()
brief = cv2.xfeatures2d.BriefDescriptorExtractor_create()

def detect_brief_features(img):
    """
    Detect keypoints and descriptors using BRIEF.
    Args:
        img: input image
    Returns:
...
def matching(descriptor1, descriptor2, feature_position1, feature_position2, pool, y_range=10):
    """
    Matching two groups of BRIEF descriptors with a y-range restriction.
    """
...
def compute_match(descriptor1, descriptor2, feature_position1, feature_position2, y_range=10):
    """
    Compute matching pairs based on BRIEF descriptors with a y-range constraint.
    """
    matched_pairs = []
    matched_pairs_rank = []
