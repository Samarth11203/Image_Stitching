...
Returns:
    A stitched image
"""
def stitching(img1, img2, shift, pool, blending=True, blending_algo="feather"):
    padding = [
    ...
    if blending:
        if blending_algo == "alpha":
            # Alpha blending
            seam_x = shifted_img1.shape[1] // 2
            tasks = [
                (shifted_img1[y], shifted_img2[y], seam_x, const.ALPHA_BLEND_WINDOW, direction) for y in range(h1)
            ]
            blended_rows = pool.starmap(alpha_blend, tasks)
        ...
        elif blending_algo == "feather":
            # Feather blending
            seam_x = shifted_img1.shape[1] // 2
            tasks = [
                (shifted_img1[y], shifted_img2[y], seam_x, const.FEATHER_WIDTH, direction) for y in range(h1)
            ]
            blended_rows = pool.starmap(feather_blend, tasks)
            shifted_img1 = np.asarray(blended_rows)
            shifted_img1 = np.concatenate((shifted_img1, splited) if shift[1] > 0 else (splited, shifted_img1), axis=1)
        else:
            raise ValueError(f"Unknown blending algorithm: {blending_algo}")
...
def feather_blend(row1, row2, seam_x, feather_width, direction='left'):
    if direction == 'right':
        row1, row2 = row2, row1
    ...
    for x in range(len(row1)):
        if x < seam_x - feather_width:
            new_row[x] = row2[x]
        elif seam_x - feather_width <= x <= seam_x + feather_width:
            alpha = (x - (seam_x - feather_width)) / (2 * feather_width)
